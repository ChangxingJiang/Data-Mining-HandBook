...
class SpiderWanplusLolDateList(tool.abc.SingleSpider):
    ...
    def running(self, start_date, end_date):
        # ----- 计算所有需要抓取的日期列表 -----
        ...
        # ----- 计算实际需要请求的日期列表 -----
        # 每一次请求均会返回该日所在周7天的数据，因此只需要请求每周的周日即可，和最早一个还没有到周日的周即可
        need_date_list = []
        # 添加之前每一周的周日
        for curr_date in all_date_list:
            if datetime.strptime(curr_date, "%Y%m%d").weekday() == 0:
                need_date_list.append(curr_date)
        # 如果最早一周还没有到周日，则添加最早一周的第一天
        if datetime.strptime(all_date_list[-1], "%Y%m%d").weekday() != 0:
            need_date_list.append(all_date_list[-1])

        # ----- 依据时间戳抓取比赛数据 -----
        for i in range(len(need_date_list)):
            print("当前请求日期:", self.format_date(need_date_list[i]), "(", i + 1, "/", len(need_date_list), ")")
...
class MySpider(SpiderWanplusLolDateList):
    def __init__(self):
    ...
    def output(self, data):
        ...
        # 生成SQL语句
        sql = ("INSERT INTO schedule_list (" + ",".join(self._COLUMNS) + ") VALUES " +
...
if __name__ == "__main__":
    print(MySpider().running(
        start_date=datetime.today() + timedelta(days=-364),  # 抓取开始日期
